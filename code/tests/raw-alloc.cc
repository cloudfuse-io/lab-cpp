...
static std::string ALLOC_TEST_NAME =
    util::getenv("ALLOC_TEST_NAME", "mmap_madv_newplace");

void mmap_munmap_inplace(size_t pagesize) {
...
void mmap_munmap_newplace(size_t pagesize) {
  int64_t allocation_size = NB_PAGE * pagesize;
  int64_t run_number = 0;
  while (run_number < 100) {
    void* region_raw_ptr =
...
void mmap_madv_inplace(size_t pagesize) {
  int64_t allocation_size = NB_PAGE * pagesize;
  void* region_raw_ptr =
      mmap((void*)(pagesize * (1 << 20)),  // Map from the start of the 2^20th page
           allocation_size,                // for one page length
...
void mmap_madv_newplace(size_t pagesize) {
  int64_t allocation_size = NB_PAGE * pagesize;
  int64_t run_number = 0;
  while (run_number < 100) {
    void* region_raw_ptr =
...
static aws::lambda_runtime::invocation_response my_handler(
  ...
  std::cout << "Alloc test " << ALLOC_TEST_NAME
            << " with system page size (bytes): " << pagesize << std::endl;
  typedef void (*pfunc)(size_t);
  ...
  (*(func_map[ALLOC_TEST_NAME]))(pagesize);
